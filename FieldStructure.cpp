...
void E_MQ_Treecode::compute_RHS_BLTC() {
    ...
    cout <<  "xmin: " << xminmax[0] << endl;
    cout <<  "xmax: " << xminmax[1] << endl;
                            ...
                            vector<vector<size_t> >& Interaction_List_near)
{
    ...
    double dfbpx = fmin(r_dist, fabs(L - r_dist));
        ...
        for (size_t ii = batch_limit_1; ii <= batch_limit_2; ii++) {
            ...
            for (size_t jj = 0; jj < far_list_size; jj++) {
                ...
                for (int kk = 0; kk < Pflat; kk++) {
                    ...
                    // double k2 = s - 0.5;
                    // double k1eps = -norm_epsL/2.0 * k2/sqrt(k2 * k2 +epsLsq);
                    // tempx += cluster_list_moments[far_index][kk] * (k1eps + k2);
                    s = s - round(s);
                    // while (s < -0.5) { s += 1.0; }
                    // while (s >= 0.5) { s -= 1.0; }
